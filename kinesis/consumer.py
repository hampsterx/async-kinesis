        ...
        async with shard["throttler"]:
            ...
            try:
                ...
                if code == "ProvisionedThroughputExceededException":
                    ...
                    log.warning(
                        "{} hit ExpiredIteratorException, recreating iterator".format(shard["ShardId"])
                    )
                    ...
                    try:
                        ...
                        log.error(f"Unexpected error recreating iterator for shard {shard['ShardId']}: {iterator_error}")
                        shard.pop("ShardIterator", None)
        ...
        for shard in shards:
            ...
            if parent_shard_id:
                ...
                if parent_shard_id not in self._shard_topology:
                    self._shard_topology[parent_shard_id] = {"children": set(), "parent": None}
