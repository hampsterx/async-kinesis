...
async def test_producer(
    ...
    try:
        ...
        if stats["records_read"] != stats["records_written"]:
            log.error(
                f"Record mismatch: expected {stats['records_written']}, "
                f"read {stats['records_read']}"
            )
            return False, None, stats
        
        log.info(
            f"Completed {stats['records_written']} records in {round(t.elapsed, 2)} seconds"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Number of shards to create (default: 1)'
    )
    ...
    parser.add_argument(
        '--processors', '-p',
        nargs='+',
        choices=['string', 'json', 'jsonline', 'jsonlist', 'msgpack', 'kpljson', 'kplstring'],
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Run without creating AWS resources'
    )
